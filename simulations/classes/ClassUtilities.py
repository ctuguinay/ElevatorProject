...
def validated(cls):
    ...
    def validation(self, *args, **kwargs):
        """
        Does most of the heavy lifting to actually get the parameters
        and validate them, along with creating fields.
        """
        
        # Uses inspect to get the parameters and passed arguments
        arguments = copy(kwargs)
        params = inspect.signature(old_init).parameters
        paramlist = list(params.keys())
        for index in range(len(args)):
            arguments[paramlist[index + 1]] = args[index]
