...
# Number of times the simulation is run for each number of people sampled.
RUNS = 50
...
def useState(timelist: TimeList, current_state: State, current_event: TimeListEvent, model: Model) -> tuple:
    ...
    if current_event.object_type == "Arrival":
        ...
        # setting the floor that the elevator currently is on.
        ...
        # Things that are specific to Hall Calls, like adding the people on the up_calls or down_calls.
        ...
        # Changes that are specific to Door Close.
    ...
    if current_state.elevator.moving:
        # The elevator is currently moving between floors so we can't do anything.
        # No need to call the Model, just return with the state we've changed.
        return timelist, current_state, added_time
    ...
    if current_state.elevator.letting_people_in:
        ...
        if current_state.elevator.going_up:
            # Let people who have up calls into elevator.
            current_state.elevator.add_floor(current_state.up_calls)
        else: # going down
            # Let people who have down calls into elevator.
    ...
    if type(command) is Idle:
        ...
        def journey_time() -> int:
            """
            Calculates the journey time of the elevator for a move command.

            Returns:
                Time in an int/float value that describes the journey time of the elevator.
            """
...
def add_to_timelist(timelist, filepath):
    ...
    Args:
        timelist: Timelist object with no hall calls.
        filepath: String that describes the filepath to the timelists.
...
if __name__ == "__main__":
    ...
    # Set data generation script path.
